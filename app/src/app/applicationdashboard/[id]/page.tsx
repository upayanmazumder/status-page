...
export default function DashboardDetailPage() {
  // Fix: useParams returns an object with key 'id' (not 'dashboardId') for this route
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        // Use a direct API endpoint for a single dashboard if available
        // Otherwise, filter from all dashboards as fallback
        let found: Dashboard | undefined;
        try {
          const res = await api.get(`/dashboards/${dashboardId}`);
          found = res.data.dashboard;
        } catch {
          // fallback: fetch all dashboards and filter
